...
#pdf("dis_dsb_exp.pdf",width = 18, height = 5)
...
pplot = ggplot()
pplot = pplot + geom_point(data=dat, aes(x=DISTANCE, y=DSB, color=EXPRESSION), shape=21, size=1, alpha = 0.25)
pplot = pplot + scale_colour_gradient2(na.value = "darkgreen", low = "darkgreen", mid = "darkgreen", high = "red", midpoint = 0, limits = c(-2.5, 5.5), breaks = c(-2, 5), labels = c(-2, 5), name = "Expression\n(log10 RPKM)", guide = "colourbar")
...
#Add CR genes
pplot = pplot + geom_point(data=datcr, aes(x=DISTANCE, y=DSB), shape=21, size=2, alpha = 1,colour="blue")
